  ...
  <div class="space-y-6">
    ...
    <div class="bg-white rounded-2xl shadow-sm p-4 border border-gray-100 flex items-center justify-between">
      <div class="flex items-center gap-3">
        <el-avatar :size="48" class="bg-red-50 text-red-600 font-bold border border-red-100">A</el-avatar>
        <div>
    ...
    <div v-for="group in menuConfig" :key="group.title" class="space-y-3">
      ...
      <div class="grid grid-cols-2 gap-3">
        ...
        <template v-if="!group.children || group.children.length === 0">
            <div class="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex flex-col items-center justify-center cursor-pointer hover:bg-gray-50 active:scale-95 active:bg-gray-100 transition-all aspect-square" @click="navigateTo(group.path)">
               <div class="w-12 h-12 rounded-2xl bg-red-50 text-red-600 flex items-center justify-center text-xl mb-3 shadow-[0_4px_12px_rgba(229,25,35,0.08)]">
        ...
        <!-- 有子菜单 -->
        <template v-else>
            <div v-for="sub in group.children" :key="sub.title" class="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex flex-col items-center justify-center cursor-pointer hover:bg-gray-50 active:scale-95 active:bg-gray-100 transition-all aspect-square" @click="navigateTo(sub.path)">
               <div class="w-12 h-12 rounded-2xl bg-red-50 text-red-600 flex items-center justify-center text-xl mb-3 shadow-[0_4px_12px_rgba(229,25,35,0.08)]">

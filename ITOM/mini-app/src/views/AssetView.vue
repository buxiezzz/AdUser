<template>
  <div class="min-h-screen bg-gray-100 font-sans text-gray-800 relative pb-20">
    ...
    <div class="p-4 pt-14">
      ...
      <el-icon class="is-loading text-blue-500 mb-4 text-3xl"><Loading /></el-icon>
    ...
    <div v-else-if="asset" class="pb-24 px-2 max-w-lg mx-auto">
       <!-- Header Card: Modern Glassmorphic Look -->
       <div class="bg-gradient-to-br from-indigo-600 via-blue-600 to-indigo-800 rounded-3xl p-6 text-white shadow-lg shadow-indigo-500/30 relative overflow-hidden mb-5">
         <div class="absolute -right-6 -bottom-6 w-32 h-32 bg-white/10 rounded-full blur-2xl"></div>
         <div class="absolute right-4 top-4 opacity-20 text-6xl pointer-events-none">✨</div>
         
         <div class="mb-6">
            <h1 class="text-3xl font-extrabold tracking-tight mb-1 break-all mr-8">{{ getCategoryName() }}</h1>
            <p class="text-indigo-200 text-sm tracking-widest font-mono">{{ asset.asset_code }}</p>
         </div>
         
         <div class="flex flex-wrap gap-2 items-center">
            <span class="px-3 py-1.5 rounded-lg text-xs font-semibold backdrop-blur-md" :class="statusClass(asset.status)">
              <div class="flex items-center gap-1">
                <div class="w-1.5 h-1.5 rounded-full bg-current animate-pulse"></div>
                {{ asset.status }}
              </div>
            </span>
            <span v-if="asset.dynamic_attributes?.['规格型号']" class="px-3 py-1.5 bg-white/10 backdrop-blur-md rounded-lg text-xs font-medium border border-white/10">
              {{ asset.dynamic_attributes['规格型号'] }}
            </span>
         </div>
       </div>

       <!-- Core Info -->
       <div class="bg-white rounded-3xl p-5 shadow-sm shadow-gray-200/50 mb-5 border border-gray-100/50 relative overflow-hidden">
         <div class="absolute top-0 right-0 w-16 h-16 bg-gradient-to-bl from-blue-50 to-transparent rounded-bl-3xl"></div>
         <div class="flex items-center mb-5">
            <div class="w-8 h-8 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center mr-3">
              <el-icon><User /></el-icon>
            </div>
            <h2 class="font-bold text-gray-800 text-lg">归属信息</h2>
         </div>
         
         <div class="grid grid-cols-2 gap-y-6 gap-x-4">
           <div class="bg-gray-50/80 rounded-2xl p-4 transition-colors hover:bg-gray-50">
             <p class="text-xs text-gray-400 mb-1.5 uppercase tracking-wider font-semibold">使用人</p>
             <p class="font-bold text-gray-800 text-base flex items-center gap-1.5">
               {{ asset.owner ? asset.owner.name : '闲置中' }}
             </p>
           </div>
           <div class="bg-gray-50/80 rounded-2xl p-4 transition-colors hover:bg-gray-50">
             <p class="text-xs text-gray-400 mb-1.5 uppercase tracking-wider font-semibold">所属部门</p>
             <p class="font-bold text-gray-800 text-base line-clamp-1">
               {{ asset.owner ? asset.owner.department : '-' }}
             </p>
           </div>
           <div class="col-span-2 bg-gray-50/80 rounded-2xl p-4" v-if="asset.dynamic_attributes?.['序列号']">
             <p class="text-xs text-gray-400 mb-1.5 uppercase tracking-wider font-semibold">硬件序列号 (SN)</p>
             <div class="flex items-center justify-between">
               <p class="font-medium text-gray-700 font-mono tracking-tight">{{ asset.dynamic_attributes['序列号'] }}</p>
               <el-button size="small" circle plain @click="copyToClipboard(asset.dynamic_attributes['序列号'])">
       ...
       <!-- Dynamic Info -->
       <div v-if="Object.keys(dynamicExtAttrs).length > 0" class="bg-white rounded-3xl p-5 shadow-sm shadow-gray-200/50 border border-gray-100/50">
         <div class="flex items-center mb-5">
            <div class="w-8 h-8 rounded-full bg-amber-50 text-amber-500 flex items-center justify-center mr-3">
              <el-icon><List /></el-icon>
            </div>
            <h2 class="font-bold text-gray-800 text-lg">规格参数</h2>
         </div>
         <div class="space-y-3">
           <div v-for="(val, key) in dynamicExtAttrs" :key="String(key)" 
                class="flex flex-col sm:flex-row sm:justify-between py-2 border-b border-gray-50 last:border-0 relative pl-4">
             <div class="absolute left-0 top-4 bottom-4 w-1 bg-gray-200 rounded-full sm:hidden"></div>
             <span class="text-sm text-gray-500 mb-1 sm:mb-0">{{ key }}</span>
             <span class="text-sm font-semibold text-gray-800 break-words sm:text-right">{{ val || '-' }}</span>
           </div>
         </div>
       </div>
       ...
       <div class="mt-10 mb-8 flex items-center justify-center">
         ...
         <span class="mx-4 text-xs font-medium text-gray-400 tracking-widest uppercase">ITOM SYSTEM</span>
         <div class="h-[1px] w-12 bg-gray-300"></div>
       </div>
    </div>
    </div>
...
import { WarnTriangleFilled, User, List, Avatar, SwitchButton, Printer, Plus, RefreshLeft, DocumentCopy, Loading } from '@element-plus/icons-vue'
